...
const ProgramDetailsEntryMassTrialTargetsPage = () => {
  ...
  const handleSubmit = () => {
    ...
    let massTrialSheetItems = [];
    for (let i = 0; i <= idx; i++) {
      for (let j = 0; j < 5; j++) {
        for (let k = 0; k < 5; k++) {
          if (targets[i][j][k].length != 0) {
            massTrialSheetItems.push({ targetName: targets[i][j][k] });
          }
        }
      }
    }

    body.massTrialSheet.massTrialSheetItems = massTrialSheetItems;
        ...
        <div className="flex flex-col justify-start max-w-[1098px] mb-[21px] mx-[auto] md:px-[20px] w-[100%]">
          <div className="data-collection">
            ...
            <Text
              ...
              variant="h4"
            >
              {`${location.state.name} (Mass Trial)`}
            </Text>
